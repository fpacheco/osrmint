...
SELECT cjson INTO TEMP vtemptable1 FROM osrmint_viaroute('SELECT id AS seq, id AS nodeid, x, y FROM viaroute ORDER BY random() LIMIT 50','http://127.0.0.1:5000/viaroute');
-- with insert into temporal table
...
SELECT cjson INTO TEMP vtemptable2 FROM osrmint_viaroute('SELECT id AS seq, id AS nodeid, x, y FROM viaroute ORDER BY random() LIMIT 500','http://127.0.0.1:5000/viaroute');
-- with insert into temporal table
INSERT INTO rjson(rjson) SELECT cjson FROM osrmint_viaroute('SELECT id AS seq, id AS nodeid, x, y FROM viaroute ORDER BY random() LIMIT 500','http://127.0.0.1:5000/viaroute');
